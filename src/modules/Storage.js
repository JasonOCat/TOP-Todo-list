...
const retrieveProjectList = () => {
  if (localStorage.getItem('projectList') !== null) {
    ...
    let tasksToDelete = ProjectList.getTodayProject()
      .tasks.flatMap((task) => task)
      .filter((task) => !DateUtils.isDateToday(task.dueDate));

    tasksToDelete.forEach((taskToDelete) => {
      ProjectList.getTodayProject().tasks.splice(
        ProjectList.getTodayProject().tasks.findIndex(
    ...
    tasksToDelete = ProjectList.getUpcomingProject()
      .tasks.flatMap((task) => task)
      .filter((task) => !DateUtils.isPresentOrFutureDate(task.dueDate));

    tasksToDelete.forEach((taskToDelete) => {
      ProjectList.getUpcomingProject().tasks.splice(
        ProjectList.getUpcomingProject().tasks.findIndex(
          (taskIndex) => taskIndex.id === taskToDelete.id
        ),
        1
      );
...
export { saveProjectList, retrieveProjectList };
